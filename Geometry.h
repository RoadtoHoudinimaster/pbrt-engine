...
namespace pbrt {
	...
	public:
		...
		//This one determine which boundary of the box has the larget boundary!
	...
	public:
		// Ray Public Methods
		//Point represent the origin and Vector3 represent the direction!
	...
	// Ray Declarations
	//for now just leave a interface for the medium!
	...
	//Union way combine the two bounding box together!
	template <typename T>
	...
	//Intersect check weather two boxes has intersection in it!
	template <typename T>
	...
	inline bool Bounds3<T>::IntersectP(const Ray& ray, float* hitt0,
		...
		for (int i = 0; i < 3; ++i) {
			...
			//Why here need to fix the boundary 
	...
	//Overlap means to one box is completely inside another one!
	template <typename T>
	...
	//Inside check if one point is inside the bounding box!
	template <typename T>
